...
def index():
    ...
    # Read in events data
    filename = os.path.join(app.static_folder, 'data', 'events.json')
    print(filename)

    birthdate_index = -1 # have not added birthdate into array yet

    with open(filename) as events_data_file:
        data = json.load(events_data_file) # load the json file with the data
